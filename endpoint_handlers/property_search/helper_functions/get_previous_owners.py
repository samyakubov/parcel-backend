...
def get_previous_home_owners(bbl:str):
    ...
    try:
        ...
        if not deed_records.empty:
            ...
            logger.info(f"--------------------Found {len(unique_owners)} unique previous owner(s) from deed records for BBL {bbl}--------------------\n")
        ...
        if mortgage_doc:
            ...
            if not mortgage_records.empty:
                ...
                logger.info(f"--------------------Found {len(unique_owners)} unique previous owner(s) from mortgage records for BBL {bbl}--------------------\n")
                return match_phone_numbers_to_owner(phone_numbers, unique_owners)
