...
def get_previous_home_owners(bbl):
    try:
        ...
        deed_records = db.execute("SELECT party_name AS owner_name FROM vm_acris_index WHERE bbl = ? AND doc_type = 'DEED' AND partytype_desc IN ('GRANTEE/BUYER', 'GRANTOR/SELLER') RDER BY recordedfiled DESC ", [bbl])
        ...
        mortgage_doc = db.execute_df("SELECT documentid FROM vm_acris_index WHERE bbl = ? AND doc_type = 'MORTGAGE' GROUP BY documentid, recordedfiled, bbl, doc_type ORDER BY recordedfiled DESC LIMIT 1", [bbl])

        if mortgage_doc:

            mortgage_records = db.execute("SELECT party_name AS owner_name FROM vm_acris_index WHERE documentid = ? AND partytype_desc = 'MORTGAGOR/BORROWER'", [mortgage_doc[0][0]])
