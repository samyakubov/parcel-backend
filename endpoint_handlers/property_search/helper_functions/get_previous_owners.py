...
def get_previous_home_owners(bbl):
    try:
        ...
        deed_records = db.execute("SELECT party_name AS owner_name FROM aggregated_acris_records WHERE bbl = ? AND doc_type = 'DEED' AND partytype_desc IN ('GRANTEE/BUYER', 'GRANTOR/SELLER') RDER BY recordedfiled DESC ", [bbl])
