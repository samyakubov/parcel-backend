...
def get_previous_home_owners(bbl:str):
    ...
    try:
        ...
        if not deed_records.empty:
            logger.info(f"Found {len(deed_records)} deed records for BBL {bbl}. Extracting unique owners.")
            ...
            logger.info(f"Found {len(unique_owners)} unique previous owner(s) from deed records for BBL {bbl}. Matching with phone numbers.")
            return match_phone_numbers_to_owner(phone_numbers, unique_owners)

        logger.info(f"No previous owners found from deed records for BBL {bbl}. Checking mortgage records.")
        ...
        if not mortgage_doc.empty:
            ...
            if not mortgage_records.empty:
                ...
                logger.info(f"Found {len(unique_owners)} unique previous owner(s) from mortgage records for BBL {bbl}. Matching with phone numbers.")
                return match_phone_numbers_to_owner(phone_numbers, unique_owners)

        logger.warning(f"No previous owners could be determined for BBL: {bbl} from available records.")
        return []

    except Exception as e:
        logger.error(f"An unexpected error occurred while getting previous home owners for BBL {bbl}: {e}", exc_info=True)
        return []