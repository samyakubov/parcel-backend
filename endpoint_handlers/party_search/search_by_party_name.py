...
def _standardize_address(address1: str, city: str, state: str, zip_code: str) -> str:
    ...
    # Normalize zip (remove invalid zips like 00000, None, nan)
    zip_norm = str(zip_code).strip()
    if zip_norm in ('00000', 'None', 'nan', ''):
        zip_norm = ''
    ...
    try:
        ...
        for address_key, group_df in address_groups:
            ...
            addr_parts = [
                ...
                str(first_row.get('party_zip', '')).strip(),
            ]
            
            # Filter out empty parts
            addr_parts_clean = [p for p in addr_parts if p and p != '']
            ...
            # 4. Get all transaction records for this person
            document_ids = group_df['documentid'].unique().tolist()
            placeholders = ", ".join(["?"] * len(document_ids))
            transactions_df = db.execute_df(
                f"SELECT * FROM aggregated_acris_records WHERE documentid IN ({placeholders}) ORDER BY documentid",
                document_ids,
            )
            records = transactions_df.fillna(value="").to_dict(orient="records")

            # 5. Extract contact info for this person
            addresses = []
            ...
            # 7. Aggregate property ownership for this person
            properties = []
            for bbl in group_df["bbl"].dropna().unique():
                bbl_records = group_df[group_df["bbl"] == bbl].sort_values("record_filed", ascending=False)
                if not bbl_records.empty:
                    latest = bbl_records.iloc[0]
                    address = f"{latest.get('prop_streetnumber', '')} {latest.get('prop_streetname', '')}".strip()
                    if latest.get('prop_unit'):
                        address += f" Unit {latest['prop_unit']}"
                    
                    properties.append(PropertyOwnership(
                        bbl=str(bbl),
                        address=address,
                        role=str(latest.get("partytype_desc", "Unknown")),
                        doc_type=str(latest.get("doc_type", "Unknown")),
                        amount=int(latest["amount"]) if latest.get("amount") else None,
                        date=str(latest.get("record_filed", ""))
                    ))

            # 8. Find co-parties for this person
            co_parties = []
            coparties_df = db.execute_df(
                ...
                LIMIT 20
                """,
                document_ids + [party_name_records]
            )
            ...
            # 9. Search for violations for this person
            name_formats = [
            ...
            violations_df = db.execute_df(
                """
                SELECT * FROM aggregated_acris_violations 
                WHERE UPPER(respondent_name) LIKE ? OR UPPER(respondent_name) LIKE ?
                """,
                name_formats
            )
            ...
            if not violations_df.empty:
                violations = violations_df.fillna(value="").to_dict(orient="records")

            # 10. Create person profile
