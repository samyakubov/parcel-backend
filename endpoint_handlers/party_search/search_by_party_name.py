...
def search_by_party_name(last_name: str, first_name: str, db: DatabaseConnector) -> dict[str, str | int | list]:
    ...
    try:
        ...
        doc_id_df = db.execute_df(
            "SELECT DISTINCT documentid FROM aggregated_acris_records WHERE UPPER(party_name) = UPPER(?)", [party_name]
        )
        ...
        logger.info(
            f"Found {len(document_ids)} document ID(s) for party name: '{party_name}'. Fetching full transaction records."
        )
        ...
        logger.info(
            f"Successfully retrieved {len(transactions_df)} transaction records for party name: '{party_name}'."
        )
        return {
