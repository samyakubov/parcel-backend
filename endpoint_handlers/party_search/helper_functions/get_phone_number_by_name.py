...
def get_phone_number_by_name(party_name: str, db: DatabaseConnector) -> pd.DataFrame | list:
    ...
    try:
        ...
        phone_numbers = db.execute_df(
            "SELECT distinct ownersphone as owners_phone, ownersfirstname as owners_first_name, ownerslastname as owners_last_name FROM dobjobs WHERE ownername LIKE ?",
            [f"%{party_name}%"],
        )
        ...
        logger.error(
            f"An unexpected error occurred while fetching phone numbers for party name '{party_name}': {e}",
            exc_info=True,
        )
        return []
